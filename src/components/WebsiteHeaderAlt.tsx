...
export type WebsiteHeaderAltType = {
  className?: string;
};
...
const NavigationItems = styled(Box)`
  ...
  gap: var(--gap-31xl);
`;
const NavigationItemsWrapper = styled(Box)`
  width: 630px;
  height: 24px;
  display: flex;
  flex-direction: row;
...
const Navigation1 = styled(Box)`
  ...
  align-items: flex-start;
  justify-content: flex-start;
  padding: var(--padding-lg) var(--padding-lgi) 0px 0px;
...
const UserActions = styled(Button)`
  ...
  z-index: 3;
`;
const WebsiteHeaderAltRoot = styled.header`
  width: 1920px;
  height: 114px;
  display: flex;
  flex-direction: row;
...
const WebsiteHeaderAlt: FunctionComponent<WebsiteHeaderAltType> = ({
  className = "",
}) => {
  return (
    <WebsiteHeaderAltRoot className={className}>
      <HeaderBackground />
    ...
    </WebsiteHeaderAltRoot>
  );
};

export default WebsiteHeaderAlt;
